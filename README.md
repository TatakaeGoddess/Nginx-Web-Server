...
```
	ufw status
```	
## 14.  Проверяем внесенные изменения в текущей конфигурации Nginx на ошибки: 
```	
 nginx -t
```	
...
```	
## 16. Проверяем корректность введенных изменений:
...
В /etc/nginx/sites-avaliable/default изменяем:
```
	server {
	        listen 8443 ssl;
	        listen [::]:8443 ssl;
...
```	
## 21. Настраиваем автостарт веб-сервера Nginx:
```
systemctl enable nginx
```
	
## 22. Настраиваем модуль статистики веб-сервера Nginx, к которому разрешаем доступ только с localhost:
...
Переходим по адресу:
https://192.168.43.108:8443/basic_status 
